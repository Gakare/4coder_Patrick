...
function Implicit_Map_Result F4_ImplicitMap(Application_Links *app,
                                            String_ID lang, String_ID mode,
                                            Input_Event *event) {
  ...
  if (GlobalKeybindingMode == KeyBindingMode_1) {
    for (int PairIndex = 0; PairIndex < ArrayCount(GlobalCommandMapReroute);
         ++PairIndex) {
  ...
  Command_Binding binding =
      map_get_binding_recursive(&framework_mapping, map_id, event);
  if (!binding.custom) {
...
// NOTE (gakare): Modal customization layer
...
CUSTOM_DOC("go to insert mode") {
  set_current_mapid(app, mapid_insert);
  active_color_table.arrays[defcolor_cursor].vals[0] = 0xff6c3a5c;
  active_color_table.arrays[defcolor_at_cursor].vals[0] = 0xff6c3a5c;
...
CUSTOM_DOC("go to normal mode") {
  ...
  active_color_table.arrays[defcolor_margin_active].vals[0] = 0xffc1ae7c;
}

//~ NOTE(rjf): Bindings
function void F4_SetAbsolutelyNecessaryBindings(Mapping *mapping) {
  ...
  mapid_normal = vars_save_string_lit("keys_normal");
...
function void F4_SetDefaultBindings(Mapping *mapping) {
  String_ID global_map_id = vars_save_string_lit("keys_global");
  String_ID file_map_id = vars_save_string_lit("keys_file");
  String_ID code_map_id = vars_save_string_lit("keys_code");
  ...
  // NOTE(rjf): Custom bindings.
  {
    ...
    Bind(f4_search_for_definition__current_file, KeyCode_J, KeyCode_Control,
         KeyCode_Shift);
    Bind(fleury_toggle_battery_saver, KeyCode_Tick, KeyCode_Alt);
    Bind(move_right_token_boundary, KeyCode_Right, KeyCode_Shift,
         KeyCode_Control);
    Bind(move_left_token_boundary, KeyCode_Left, KeyCode_Shift,
         KeyCode_Control);
  }
  ...
  Bind(snipe_backward_whitespace_or_token_boundary, KeyCode_Backspace,
       KeyCode_Alt);
  ...
  Bind(list_all_locations_of_selection, KeyCode_G, KeyCode_Control,
       KeyCode_Shift);
  ...
  Bind(list_all_locations_of_identifier, KeyCode_T, KeyCode_Control,
       KeyCode_Shift);
  ...
  Bind(view_jump_list_with_lister, KeyCode_Period, KeyCode_Control,
       KeyCode_Shift);
  ...
  Bind(list_all_locations_of_type_definition_of_identifier, KeyCode_T,
       KeyCode_Alt, KeyCode_Shift);
  Bind(open_long_braces, KeyCode_LeftBracket, KeyCode_Control);
  Bind(open_long_braces_semicolon, KeyCode_LeftBracket, KeyCode_Control,
       KeyCode_Shift);
  Bind(open_long_braces_break, KeyCode_RightBracket, KeyCode_Control,
       KeyCode_Shift);
  Bind(select_surrounding_scope, KeyCode_LeftBracket, KeyCode_Alt);
  Bind(select_surrounding_scope_maximal, KeyCode_LeftBracket, KeyCode_Alt,
       KeyCode_Shift);
  Bind(select_prev_scope_absolute, KeyCode_RightBracket, KeyCode_Alt);
  Bind(select_prev_top_most_scope, KeyCode_RightBracket, KeyCode_Alt,
       KeyCode_Shift);
  Bind(select_next_scope_absolute, KeyCode_Quote, KeyCode_Alt);
  Bind(select_next_scope_after_current, KeyCode_Quote, KeyCode_Alt,
       KeyCode_Shift);
